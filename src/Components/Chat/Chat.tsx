...
const Chat:React.FC<role>=({role})=> {
  ...
  const setMessageFn = (e: React.ChangeEvent<HTMLInputElement>) => {
   ...
   const handleSendMessage=async () => {
    ...
    //   const res = await sendMessage(newMessage, userId,companyId, currentUserId)
    //   setNewMessage('')
    //   console.log(res.msg);
   ...
   const sendMessage=async(content:string,chatId:string,userId:string,cmpId:string)=>{
    const {data}= await api.post(`/message/send`,{content,chatId,userId,cmpId})
    return data
}
  useEffect(()=>{
        ...
        <div className="w-full">
          ...
          <div className="flex items-center justify-between w-full p-3 border-t border-gray-300">
            ...
            <input type="text" placeholder="Message"
              ...
              name="message" value={newMessage} onChange={setMessageFn} required />
            <button>
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-gray-500" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
